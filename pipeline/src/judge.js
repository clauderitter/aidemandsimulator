...
function leastDistinct(state, camp) {
  ...
  const all = state.scenarios.filter(s => s.status !== 'retired'); const act = all.filter(s => !s.core && s.id !== 'base'); if (!act.length) return null; const paths = Object.fromEntries(all.map(s => [s.id, path(state, s)]));
  const nn = s => Math.min(...all.filter(o => o.id !== s.id).map(o => gap(paths[s.id], paths[o.id])));
