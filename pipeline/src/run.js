...
if (!only || only === 'memo') {
  const monday = new Date().getUTCDay() === 1; const want = monday || process.env.MEMO === '1';
  const can = cfg.agents_enabled && (process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_AUTH_TOKEN || process.env.AGENTS_MOCK === '1');
