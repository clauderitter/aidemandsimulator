...
export async function gatherItems(cfg) {
  ...
  const health = readJSON(P('pipeline', 'state', 'feed_health.json'), []).filter(h => h.date >= new Date(Date.now() - 14 * 86400000).toISOString().slice(0, 10)); health.push({ date: new Date().toISOString().slice(0, 10), failed: failed.filter(f => !f.dead).map(f => f.name), dead: failed.filter(f => f.dead).map(f => f.name) }); writeJSON(P('pipeline', 'state', 'feed_health.json'), health);
